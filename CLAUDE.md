...
UI is a 4-pane IDE layout: top command band (＋ Add-table modal, ⌕ Ctrl+K command palette, ⚙ Document popover — global body font / line spacing / label separator / indent / reset), left **Sections rail** (each table = its OWN standalone Word document; drag-reorder, ⧉ duplicate, ✕ remove), center builder (three cards by SCOPE: Section Title / **Rows** per-field / **Level formatting** per-level), pinned right preview (Preview on a true-to-scale Word page / Table). Intake is the ＋ Add-table modal (paste / drop a file / example, previewed) + paste-anywhere; a fresh paste is auto-arranged by Smart Arrange. Export is per-section **Copy section** (clipboard only). Workspace auto-saves to `localStorage` (local-only); undo/redo; first-run onboarding.
...
- `components/TableCard.tsx` — center builder, THREE cards split by SCOPE (deliberate: Rows' Aa/B/I/U touch only the LABEL before the colon, a level's Look restyles the WHOLE line): **Section Title** (title text + Word-heading dropdown incl. a **Custom style…** text input for any destination style name + shared title look) · **Rows** = per FIELD (Add-fields pool; ONE row per field with tree guides, a live micro-preview in the level's real look + sample value, label/sort toggles, and ◄ ► ▲ ▼ ✕ buttons — drag-and-drop was built here and REVERTED; `flashRow`/`applyMove` briefly tint the changed row) · **Level formatting** = per LEVEL (Markers mode + blank-line/page-break checkboxes, then a matrix: Level | Marker split **Type + Delimiter** (delimiter disabled for symbol/none, "None" spelled out; "Word numbers" note on heading levels) | Word heading + Auto/H1–H9 rank chip | **Look** = the SAME `LookControl` swatch+Aa▾ the Section Title uses, badged *all tables*); a stacked sibling shows a "shares level N" note
