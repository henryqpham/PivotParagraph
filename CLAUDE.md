...
- `components/TableCard.tsx` — center builder, THREE cards split by SCOPE (deliberate: Rows' Aa/B/I/U touch only the LABEL before the colon, a level's Look restyles the WHOLE line): **Section Title** (title text + Word-heading dropdown incl. a **Custom style…** text input for any destination style name + shared title look) · **Rows** = per FIELD (Add-fields pool; ONE row per field with tree guides, a live micro-preview in the level's real look + sample value, label/sort toggles, and ◄ ► ▲ ▼ ✕ buttons — plus an OPT-IN drag-to-reorder toggle in the card header (`dragRows`, default off): a ⋮ grip per row, VERTICAL-only via `reorderField` (fixed indent skeleton — a drop can't indent/stack), ▲ ▼ hidden while on; `flashRow`/`applyMove` briefly tint the changed row) · **Level formatting** = per LEVEL (Markers mode + page-break checkbox, then a matrix: Level | Marker split **Type + Delimiter** (delimiter disabled for symbol/none, "None" spelled out; "Word numbers" note on heading levels) | **Blank line** per level (`breakAfter[i]`) + **Gap after** per level (`gapAfter[i]`, after the whole group) | Word heading + Auto/H1–H9 rank chip | **Look** = the SAME `LookControl` swatch+Aa▾ the Section Title uses, badged *all tables*); a stacked sibling shows a blank number pill (tree guides carry the nesting). Blank line = spacer AFTER the level's own line, BEFORE its children
