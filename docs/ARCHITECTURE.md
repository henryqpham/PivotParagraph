...
**Copy all** is the sole export: it joins every table's `tableToHtml` fragment (in paste order) and runs **one** `buildWordHtml` (valid because its rewrites are global regexes and it emits a single `@page`). `tableToHtml` still powers the live preview, so preview and export stay identical. `renderPivotTree` escapes all user-derived text (`& < >`). The JSON view shows the raw Grid instead of the rendered tree.
